install.packages("rmarkdown", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("bookdown", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("janitor", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("ineq", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("tidyverse", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("caret", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("rpart.plot", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("kableExtra", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("usmap", repos='http://cran.uni-muenster.de/', dependencies=TRUE)
install.packages("randomForest", repos='http://cran.uni-muenster.de/', dependencies=TRUE)

update.packages(repos='http://cran.uni-muenster.de/', dependencies=TRUE, ask=FALSE, checkBuilt=TRUE)

