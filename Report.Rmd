...
*To execute the provided you may need to enter your Python path in a R coding block by "use_python()". If this does not work, copy the Python coding into the Python IDE of your choice, ideally under the recommended location, which is mentioned at the top of each code block. The path is always relative to the repository.*
...
ggplot() + 
  ...
  theme(axis.line=element_blank(),axis.text.x=element_blank(),
        ...
        axis.title.x=element_blank(), axis.title.y=element_blank(),
        panel.background=element_blank(),panel.border=element_blank(),panel.grid.major=element_blank())
ggsave(filename=paste(pathPlotFolder,"Customer Data Plots/Cities_weighted.png",sep=""), width=10)
...
ggplot() +
  ...
  theme(axis.line=element_blank(),axis.text.x=element_blank(),
        ...
        axis.title.x=element_blank(), axis.title.y=element_blank(),
        panel.background=element_blank(),panel.border=element_blank(),panel.grid.major=element_blank())
ggsave(filename=paste(pathPlotFolder,"Customer Data Plots/Cities.png",sep=""), width=15)
#-----------------------------------------------------------------------------------
#age data (weighted)
...
Following the inference analysis our next task is to predict whether or not a person browsing the online-shop will end up purchasing something, based on the browsing behaviour on the site. Since only the behaviour should be regarded for the predictions, we left out any customer information. But it should be regarded that an includence of these information could improve the performance of the prediciton model.
...
However an important thing to note is that about 99% of sessions do not contain an order. In addition to that, sessions consisting of only one click are ignored, since they offer no valuable information, because by example attributes like the session duration cannot be calculated.

After preparing the data the next step is to train and test a model. The model of choice is a decision tree, since it allows to easily understand the decision making process of the model. Since it removes the need for dimensionality reduction and feature scaling, it also reduces the required work. The evaluation metric used during the grid search is ROC-score. However, depending on the evolving needs of the company, one might want to chose a different evaluation metric.
...
plt.savefig(pathConfusionMatrix, cmap='gray')
plt.show()
