...
author: "Kim Ferres, Julian Cornea, Jan Löffelmann"
date: "23-05-2019"
output:
...
```{r importingMyLibraries, echo=F, results = "hide", message=F, warning=F}
  ...
  library(knitr)
```
#About of the Data {#data}

The data comes from an online shop that sells beauty products. There are two datasets: one with data about customer orders and one with data about customer clicks on the website.

The customer orders dataset contains data such as the number of product units ordered, the total order amount, payment information, the manufacturer and brand names of the ordered product and social data about the customer, like his location and age. The dataset contains one row for each order.

The customer clicks dataset contains data such as the timestamp of a click, the session length and the name of the product that was clicked as well as the product category it belongs to. The dataset contains one row for each click.

Both datasets share a considerable amount of columns, however since not every click results in an order and since a session can be made up of more than one click, the contents differ significantly.

#Task Description {#task}

The task of this project was to analyze the dataset and create plots and statistical tables for the data, that is suspected to be relevant for the online shop. 
...
Before we started cleaning the data we copied the data to the folder "0 Data". The reason behind this is that we want to avoid altering the original datasets. The cleaned data was also saved in this folder. Before explaining the cleaning process it makes sense to get an overview of the content of the aforementioned folder. It can be described as follows:

* Each file has a suffix depending on what language was used for creating it. Files created with a Python script have the suffix "_P". Files created with R have the suffix "_R". 
* For both datasets three types of files are created: 
    ...
    + A smaller version of the clean version to allow quick testing of the technical functionality of code (e.g. "order_data_small_R.csv")
...
The aim of the data analysis is to extract information, which is suspected to be valuable to the online shop, and prepare it in a way that makes it easily "digestible". The overview of the information is presented in one of two ways:

* A statistical table. Given a subset of interesting columns we create two statistical tables: one for numerical columns in the subset and one for factors. The statistical table for the numerical columns contains the maximum value, minimum value, mean, median and standard deviation for each numerical column. The other one contains the five most frequent factors as well as the percentage of NAs and other factors for each factor column.
* A plot. Some information is too complex to be compressed into a single table without making it too confusing, or it's simply easier to understand if presented as a plot. Some of the plot types used are time series plots, stacked bar plots, lorenz curve plots and maps.

##How much Data is Missing? {#missingData}

Before creating overview tables or plots for columns it makes sense to look which columns actually contain a large quantity of information and which dont. To do that we created a ranking for both datasets containing column names and the percentage of missing data for each column. Columns with a low percentage of missing data are then preferred in later analysis steps. The resulting rankings can be seen in the following two code blocks.
