...
The provided data comes from an online shop selling beauty products. There are two datasets given: One with data about customer orders and another with data about customer clicks on the website.  
The orders dataset consists of one row for each customer order in the time periode from the 28. January 2000 to the 3. March 2000. The data contains values characterizing the ordered products, the used payment methods, the order process itself and social data, as by example the customer's location or age.  
...
*Note: As we explain in this subchapter, the merged data is too small to be beneficial for any useful purpose. This is why we did not implement the merging in R after the coding in Python.*
...
2) **Product Data**: The following data columns describe features of products clicked on by the customers. Similar to the customer data, the product data listing is cutted to the most essential attributes.
...
*Note: To support a better visualization, the most relevant columns are highligthed in black color.*
...
When comparing the product and customer information for the order and click dataset directly some intersting observations can be made.

#### Customer Data

**Age:** The summarised age values of the click and order dataset are similar, but nor entirely equal. The median is indeed equal for both datasets. However, the mean differs a little and is lower for the click data, indicating that young people might browse more than buy.  
...
**City:** From the following overview can be observed that especially people from New York tend to order relativly often compared to their browsing amount. But since a lot of the click data for the cities is not available, the comparison is possibly not that meaningful.  
...
**Audience:** The confrontation of the audience for both datasets displays that more women and children products get viewed than buyed, whereas the opposite can be said for men products. Since it can be assumed that usually women look for children products, this observation shows that male users are the more target buyers. This could be interesting by example for discount offerings, because an indecisive potential customer is more likely to be affected by such offers than a determined one. 
...
```
<br><br><br><br><br><br><br>
Whereas the brand lorenz curves shows a more equal distributed popularity, because it is overall rather flat. At about 50% of the brands, a pointy part can be seen in the graph, which devides the brands in a rather popular half and a rather unpopular one.
...
The following plot shows that most of the clicks are accumulated in the morning and then descending over day. The extreme spike at around 2 a.m. can be neglected as it seems to be bot activity with extremly long session times and sequences.  
...
The maps clearly show the numerical difference in the both dataset. Especially Chicago and Dellas have to be highlighted because both this cities have a really high viewer amount, but a rather small customership.
...
First of all the relevant data had to be extracted or engineered from the clickstream data and was then used to build a seperate dataset, which represents the input data for a prediction model. The resulting data set contains one row per session. For each session the following attributes are used:
...
The second model of choice is a random forrest. A random forrest is an ensemble model made up of several decision trees. Due to the structure of a random forrest it has less bias than a single decision tree, but it suffers from higher variance. It also offers less insight into the logic of the model, since it is made up of several trees. However, depending on the forrest complexity, it is still possible to present an overview of that logic. For example if the forrest is made up of only 5 trees, each tree could be visualized. However, the more trees a forrest is made out of, the less transparency it offers.
...
This shows a 1%-3% percentage drop in performance compared to the previous decision tree model. This indicates a high variance in the data, which favors a decision tree more than a random forrest.
...
The clickstream dataset contains a high quantity of information. Thus, not all attributes are displayed in the summary tables above, but can be seen in the followiing overview tables.
