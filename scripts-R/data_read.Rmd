...
t.test(experiment$ranking_based, experiment$random_recommendations, alternative="greater")
t.test(experiment$profit_oriented, experiment$random_recommendations, alternative="greater")
```

```{r}
# generate data
a <- rnorm(100000,mean(experiment$profit_oriented), sd(experiment$profit_oriented)) %>% 
  sort()
b <- rnorm(1000,mean(experiment$ranking_based), sd(experiment$ranking_based)) %>%
  sort()
c <- rnorm(1000,mean(experiment$random_recommendations), sd(experiment$random_recommendations)) %>%
  sort()


# plot data
ggplot()+
  geom_density(aes(x=experiment$profit_oriented),adjust=1, color="red")+
  geom_density(aes(x=experiment$random_recommendations),adjust=1, color="blue")+
  geom_density(aes(x=experiment$ranking_based),adjust=1, color="green")

ggplot()+
  geom_density(aes(x=a),adjust=0.5, color="red")+
  geom_density(aes(x=b),adjust=0.5, color="blue")+
  geom_density(aes(x=c),adjust=0.5, color="green")
```
