...
## comapre py csv and r csv
``` {r}
write.csv(dataframe, file = "../00_raw_data/order_r.csv", row.names =FALSE, na = "")

# r csv
r_df = read.csv(file = "../00_raw_data/order_r.csv",na=c("", "?", "NULL", "NA", "Nan") )

# py csv
py_df = read.csv(file = "../00_raw_data/order_py.csv",na=c("", "?", "NULL", "NA", "Nan"))


head(r_df)
head(py_df)
# show difference between python and r dataset
diff <-r_df[!(r_df%in%py_df)]
head(diff)
